...
class collinear
{
   ...
   // ----------------------------------------------------------
     ...
     *
     *  Explanation: 2 inner for-loops implemented with binary search's order of growth
     *
     *
     */
    static int countCollinearFast(int[] a1, int[] a2, int[] a3)
    {
      ...
      for (int i = 0; i < a1.length; i++)
      {
    	  ...
    	  for (int j = 0; j < a2.length; j++)
    	  {
    		 x2 = a2[j];
    		 thirdMatch = ((x2-x1)+x2);
    		 if (collinear.binarySearch(a3,thirdMatch) == true)
    		 {
    	   		 counter++;
    		 }
    	  }
     ...
     *
     *  Explanation: Divide and Conquer method, time goes up linearly while the n goes up exponentially 
     *
     */
    static boolean binarySearch(int[] a, int x)
    {
      int lowPoint =0, highPoint = a.length-1;
      while (lowPoint <= highPoint) {
    	  int middlePoint = lowPoint + (highPoint - lowPoint)/2;
    	  if (x < a[middlePoint]) highPoint = middlePoint -1;
    	  else if ( x > a[middlePoint]) lowPoint = middlePoint + 1;
    	  else return true;
      }
      return false;
    }
